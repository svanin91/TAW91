...
const ModificaUtente: React.FC = () => {
  ...
  const [valueCognome, setValueCognome] = useState(datiUtente.M_02_cognome);
  const [valueNome, setValueNome] = useState(datiUtente.M_01_nome);
  const [valueGrado, setValueGrado] = useState(datiUtente.M_03_grado);
  const [valueMatricola, setValueMatricola] = useState(
    datiUtente.M_04_matricola
  );
  const [valueTelefono_1, setValueTelefono_1] = useState(
    datiUtente.M_15_telefono_uno
  );
  const [valueTelefono_2, setValueTelefono_2] = useState(
    datiUtente.M_16_telefono_due
  );
  const [valueReparto, setValueReparto] = useState(datiUtente.M_10_reparto);
  const [valueIndirizzo, setValueIndirizzo] = useState(
    datiUtente.M_11_indirizzo
  );
  const [valueSNChiave, setValueSNChiave] = useState(datiUtente.M_12_sn_chiave);
  const [valueSMSBook, setValueSMSBook] = useState(datiUtente.M_13_SMSBook);
  const [valueAttivo, setValueAttivo] = useState(datiUtente.M_07_attivo);
  const [valueNomeUtente, setValueNomeUtente] = useState(
    datiUtente.M_05_nome_utente
  );
  const [valueEmail, setValueEmail] = useState(datiUtente.M_14_email);

  const UtenteMyContext = useContext(UtenteContext);

  useEffect(() => {
    UtenteMyContext.setA4_03_TabRig_Utenti_M_01_nome(datiUtente.M_01_nome);
    UtenteMyContext.setA4_03_TabRig_Utenti_M_02_cognome(
      datiUtente.M_02_cognome
    );
    UtenteMyContext.setA4_03_TabRig_Utenti_M_03_grado(datiUtente.M_03_grado);
  ...
  const handleChangeReparto = (e: React.ChangeEvent<HTMLSelectElement>) => {
    UtenteMyContext.setA4_03_TabRig_Utenti_M_10_reparto(e.target.value);
    setValueReparto(e.target.value);
  };
  ...
  //FUNZIONE DI CONTROLLO E INVIO I DATI AL DATABASE PER MODIFICARE IL RECORD
  const submitData = async () => {
    ...
    try {
      await axios.patch("http://localhost:10200/modifica-utente", {
        dataA4_03_TabRig_Utenti,
        id_record: datiUtente.id_record,
      });
    } catch (error) {
      console.log(error);
    }
    ...
    >
      <div className="h-fit flex justify-between items-center ">
        <p className="text-2xl ...">
          MODIFICA UTENTE : {datiUtente.M_05_nome_utente}
        </p>
        <div>
          <button
            onClick={() => submitData()}
        ...
        <form className="m-10">
          <div className="grid gap-6 mb-6 md:grid-cols-2">
            <div>
              ...
              <input
                value={UtenteMyContext.A4_03_TabRig_Utenti_M_02_cognome}
      ...
      {showPopUpEsito && (
        <EsitoPopUp
          title={"OTTIMO !!"}
          message={`Utente ${UtenteMyContext.A4_03_TabRig_Utenti_M_05_nome_utente} modificato con successo`}
